...
inputImg = cv2.imread('dengarkan.png') #PerangJohor-4.png') #p01-lineimg0_n0002_label29.png') #'topanribut.png')  ganti yg lain
#coba data training p01-lineimg0_n0003_label01.png potongan angle di pinggir
input2 = cv2.imread('dengarkan.png') #PerangJohor-4.png') #p01-lineimg0_n0002_label29.png') #'topanribut.png')
...
_, binarizedImg = cv2.threshold(gaussianFilter, 0, 180, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
...
#U = [[],[]]
U = [[list_space_angle_start[0][0] , list_space_angle_start[0][1] ],[list_space_angle_end[0][0] , list_space_angle_end[0][1] ]]        
...
res = np.asarray(list_split)   #list hasil sbelum rotate  #ralat:BELUM, res dari list_start dan list_end
...
resmul = np.matmul(res, rotation_matrix)  #multiply for rotation transform for the start points of red lines
...
rotated_image = cv2.warpAffine(image, rotation_matrix, (width, height))     #  dari list  split        #crop, rotation_matrix, (width, height))
