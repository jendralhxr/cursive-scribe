...
train_images = (
    train_images.reshape((train_images.shape[0], IMG_WIDTH, IMG_HEIGHT, 1)) / 255.0
)
test_images = (
    test_images.reshape((test_images.shape[0], IMG_WIDTH, IMG_HEIGHT, 1)) / 255.0
)
...
model.add(
    layers.Dropout(0.3)
)  # so as to avoid overfitting, some would suggest to be more aggressive
...
)  # `values_format='d'` to display integer counts
