...
def search_list_area():  # cari list angle sebelum gambar diputar
    ...
    for r in range(len(list_space_angle_start)):
        ...
        x55 = (
            -1 * (list_space_angle_start[r][1] - center[1]) * np.cos(deg / 180 * np.pi)
        )
        x55_end = (
            -1 * (list_space_angle_end[r][1] - center[1]) * np.cos(deg / 180 * np.pi)
        )
...
for s in range(len(list_space_angle_start)):
    ...
    imageln = cv.line(
        image, (list_space_angle_start[s]), (list_space_angle_end[s]), (0, 0, 255), 1
    )
...
for n in range(scribe.number_of_nodes()):
    ...
    if True:
        ...
        for i in range(len(components)):
            print(
                "indbts[components[i].nodes[-1]] ",
                indbts[n],
                indbts[components[i].nodes[-1]],
            )
            print(
                f"i {i},nodes {components[i].nodes}  components[i].centroid {components[i].centroid} "
            )
        ...
        if found == 0:  # if not found
            ...
            components.append(
                ConnectedComponents(box, pos[n])
            )  # component baru, belum ada node nya
...
for k in range(len(components)):
    ...
    if intersect == True and components[k].area > pow(SLIC_SPACE, 2) * pow(
        ...
        graph = extract_subgraph(
            scribe, components[k].node_start
        )  # ada hasil empty #empty error ?
        print("Err4 graph k, nodestart ", k, components[k].node_start, graph)
        for n in graph.nodes:
            print(
                f"graph.nodes {n} , pos[n][0] {pos[n][0]} components[k].centroid[0] {components[k].centroid[0]} SLIC_SPACE {SLIC_SPACE} "
            )
            if pos[n][0] > (components[k].centroid[0] - SLIC_SPACE):
                print(">>", n)
        print(
            "Err3 components[k].rect[3]-[2] ",
            components[k].rect[3],
            components[k].rect[2],
        )
