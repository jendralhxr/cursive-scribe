...
def search_area(point) : #(x,y) sebelum gambar diputar, untuk satu point saja
    ...
    if point[0] <=  list_space_angle_start[0][0] :  #masuk area
        for s in range(len(list_space_angle_start)):
            if  point[0] <= list_space_angle_start[s][0] and point[0] > list_space_angle_start[s+1][0]:
                #update batas atas
...
for n in range(num_slic):
    if ( len(moments[n])>SLIC_SPACE ): # remove spurious superpixel with area less than 2 px 
        ...
        if (cue[cy,cx]!=0):
            ...
            print("search n,cx,cy, idx batas1_x ",n,cx,cy,inde_bts,list_space_angle_start[inde_bts][0])    
