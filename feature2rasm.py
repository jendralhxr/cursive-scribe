...
for n in range(len(moments)):
    ...
    if len(moments[n]) != 0:
        if abs(moments[n][0][0] - moments[n][-1][0]) > SLIC_SPACE * pow(PHI, 2) or abs(
            moments[n][0][1] - moments[n][-1][1]
        ) > SLIC_SPACE * pow(PHI, 2):
            moments[n] = []
...
def close_components(b, comps, length=4):
    ...
    for i in range(length):
        ...
        if (b - i) >= 0 and (
            b - i
        ) != b:  # Ensure the value stays within range and is not b
            result.append(b - i)
        # Try to add i+1 to b
        if (b + i) < len(comps) and (
            b + i
        ) != b:  # Ensure the value stays within range and is not b
            result.append(b + i)
    return result
...
for n in range(scribe.number_of_nodes()):
    ...
    if scribe.has_node(n):
        ...
        mu = cv.moments(ccv)
        if (
            mu["m00"] > pow(SLIC_SPACE, 2) * PHI
        ):  # minimum area for a connectedcomponent
            mc = (
                int(mu["m10"] / (mu["m00"])),
                int(mu["m01"] / (mu["m00"])),
            )  # centroid of the flood fill
            area = mu["m00"]
...
for k in range(len(components)):
    ...
    for m in components[k].nodes:
        ...
        for n in components[k].nodes:
            ...
            if m != n:
                ...
                linepart = max(
                    line_iterator(gray, psrc, pdst) for psrc in fsrc for pdst in fdst
                )
    ...
    for m in components[k].nodes:
        ...
        for i in range(3):
            if ndst[i] != -1:
                nvane[i] = freeman(
                    pos[ndst[i]][0] - pos[m][0], -(pos[ndst[i]][1] - pos[m][1])
                )
        # print(f'{m} to {ndst[0]}({ndist[0]:.2f}) {ndst[1]}({ndist[1]:.2f}) {ndst[2]}({ndist[2]:.2f})')

        # excessive fork, usually at the start/end of stroke
        if (
            scribe.has_edge(m, ndst[2])
            and scribe.has_edge(m, ndst[1])
            and scribe.has_edge(ndst[2], ndst[1])
            and nvane[2] == nvane[1]
        ):
        ...
        if scribe.has_edge(m, ndst[0]) and (
            (scribe.has_edge(ndst[2], ndst[0]) and nvane[2] == nvane[0])
            or (scribe.has_edge(ndst[1], ndst[0]) and nvane[1] == nvane[0])
        ):
...
for k in range(len(components)):
    ...
    if (
        intersect == True
        and len(components[k].nodes) >= 2
        and (max_distance > SLIC_SPACE * PHI or len(components[k].nodes) >= 3)
        and components[k].area > pow(SLIC_SPACE, 2) * pow(PHI, 3)
    ):  # some valid single-letter rasm can be quite small
        ...
        scribe_dia.nodes[components[k].node_start][
            "color"
        ] = "#F00000"  # initialize with red, just in case we fail to find one
        ...
        if components[k].rect[3] / components[k].rect[2] > pow(PHI, 2):
            ...
            stroke_baseline = sum(pos[node][1] for node in components[k].nodes) / len(
                components[k].nodes
            )
            ...
            rightmost_nodes_filtered = [
                node
                for node in rightmost_nodes
                if abs(pos[node][1] - lead_baseline) < SLIC_SPACE * pow(PHI, 3)
            ]
            rightmost_nodes_filtered = sorted(
                [node for node in rightmost_nodes_filtered],
                key=lambda node: pos[node][1],
            )[: int(RASM_CANDIDATE / PHI)]
            if len(rightmost_nodes_filtered) == 0:
                ...
                adjdiff = [
                    abs(
                        pos[rightmost_nodes_filtered[i + 1]][1]
                        - pos[rightmost_nodes_filtered[i]][1]
                    )
                ...
                if max(adjdiff) > SLIC_SPACE * pow(PHI, 2):
                    ...
                    node_start = min(
                        (
                            node
                            for node in rightmost_nodes_filtered
                            if abs(pos[node][1] - lead_baseline)
                            <= SLIC_SPACE * pow(PHI, 2)
                        ),
                        key=lambda node: pos[node][1],
                    )
                    ...
                    node_start = min(
                        rightmost_nodes_filtered,
                        key=lambda node: pdistance(components[k].rightmost, pos[node])
                        * PHI
                        + (pos[node][1] - lead_baseline),
                    )
    ...
    # rather stumpy
        ...
        intersect == False
        and max_distance < SLIC_SPACE * pow(PHI, 2)
                ...
                for m in components[k].nodes:
                    for n in components[l].nodes:
                        ...
                        if (
                            ...
                            and scribe_dia.nodes[n]["rasm"] == True
                        ):
                            closest_comp = l
                            src_node = m
                            closest_node = n
    ...
    # treat small or 'crumpled' ones as diacritics
        ...
        len(components[k].nodes) <= 2
        or max_distance < SLIC_SPACE * PHI + SLIC_SPACE / 2
    ):
        for j in components[k].nodes:
            scribe_dia.nodes[j]["rasm"] = False
            scribe_dia.nodes[j]["color"] = "#008888"
...
for i in range(len(components)):
    for j in close_components(i, components):
        ...
        if (
            dia_dist < SLIC_SPACE * PHI
            and scribe_dia.nodes[components[i].nodes[0]]["rasm"] == False
            and scribe_dia.nodes[components[j].nodes[0]]["rasm"] == False
        ):
            components[i].area = components[i].area + components[j].area
            components[i].nodes = np.unique(
                components[i].nodes + components[j].nodes
            ).tolist()
            for n in components[i].nodes:
                # diacritics over
                if scribe_dia.nodes[components[i].nodes[0]]["dia_size"] in {
                    "A",
                    "B",
                    "C",
                }:
                    if components[i].area > pow(SLIC_SPACE, 2) * pow(PHI, 4):
                        ...
                        scribe_dia.nodes[n][
                            "dia_size"
                        ] = "B"  # approx pow(SLIC_SPACE,2)*pow(PHI,3)
                # diacritics under
                elif scribe_dia.nodes[components[i].nodes[0]]["dia_size"] in {
                    "a",
                    "b",
                    "c",
                }:
                ...
                if scribe_dia.nodes[components[j].nodes[0]]["rasm"] == True:
                    for m in components[i].nodes:
                        for n in components[j].nodes:
                            tdist = pdistance(pos[m], pos[n])
                            tvane = freeman(
                                pos[n][0] - pos[m][0], pos[n][1] - pos[m][1]
                            )
                            if (
                                tdist < closest_dist
                                and tdist < SLIC_SPACE * pow(PHI, 3)
                                and (tvane == 2 or tvane == 6)
                            ):
                                closest_comp = j
                                src_node = m
...
components = sorted(
    components,
    key=lambda n: n.rightmost[0]
    if n.rect[2] < SLIC_SPACE * pow(PHI, 3)
            ...
            if y_pos < ccv.shape[0] and x_pos < ccv.shape[1]:
                ...
                if ccv[y_pos][x_pos][0] == STROKEVAL:
                    ...
                    if active_stroke == True:
                        ...
                        if (
                            scribe_dia.nodes[transition_node]["rasm"] == True
                            and scribe_dia.nodes[transition_node]["color"] != "#F00000"
            ...
            for neighbor in G.neighbors(current_node):
                if neighbor not in visited:
                    ...
                    distance = pdistance(pos[current_node], pos[neighbor]) / PHI - (
                        pos[neighbor][0] - pos[current_node][0]
                    )  # prioritize right-er nodes
                    if distance < 0:
                        distance = 0
    ...
    for n in path:
        if G.has_edge(n[0], n[1]):  # ideally not necessary
            ...
            if (
                src_prev != -1
                and dst_prev != -1
                and G.has_edge(src_prev, src) == False
                and G.has_edge(dst_prev, dst) == False
                and G.has_edge(dst_prev, src) == False
                and G.has_edge(src_prev, dst) == False
            ):
                pathstring += "x"
...
for i in range(len(components)):
    rasm = ""
    if (
        components[i].node_start != -1
        and scribe_dia.nodes[components[i].node_start]["rasm"] == True
    ):
        rasm = ""
        remainder_stroke = ""
