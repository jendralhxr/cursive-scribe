...
source = source[
    (source[fieldhurf] != "")
    & ((source[fieldstring] != "") | (source[fieldstring2] != ""))
]
# some basic checks
source = source[
    source[fieldhurf].notna()
    & (source[fieldstring].notna() | (source[fieldstring2].notna()))
]
source[fieldval] = source[fieldhurf].apply(map_huruf_to_val)

# merging multiple chaincode columns
source_merged = pd.melt(
    source,
    id_vars=["huruf", "val"],
    value_vars=[fieldstring, fieldstring2],
    var_name="chaincode_id",
    value_name=fieldstring_merged,
)
source_merged = source_merged.drop(columns=["chaincode_id"])
source = source_merged[
    source_merged[fieldstring_merged].notna() & source_merged[fieldval].notna()
]
...
plt.hist(
    char_lengths,
    bins=range(min(char_lengths), max(char_lengths) + 1, 1),
    edgecolor="black",
)
...
hist_charlen_val, hist_charlen_bin = np.histogram(
    char_lengths, bins=np.arange(min(char_lengths), max(char_lengths))
)
...
tokens = {
    f"{i}": [] for i in range(0, NUM_CLASSES)
}  # substring/substroke/subchain/subsequence
    ...
    if hurf_class in tokens:
        for subsequence in tokens[hurf_class]:
            ...
            if subsequence["seq"] == rasm_seq and exact == True:
                ...
                subsequence["score"] += pow(
                    PHI, len(rasm_seq) / LENGTH_MIN / pow(PHI, 2)
                )
        ...
        if exact == True:
            tokens[hurf_class].append(
                {
                    "seq": rasm_seq,
                    "freq": 1,
                    "score": pow(PHI, len(rasm_seq) / LENGTH_MIN / PHI),
                }
            )
...
def parse_chaincode(input_string):
    ...
    for i in range(1, len(input_string_nohist)):
        ...
        # changing radical
            ...
            if (
                len(current_group) >= SUBSTROKE_MIN_LENGTH
            ):  # Ensure group has at least 4 chars
    ...
    for i in range(len(substrs)):
        if len(substrs[i]) == 1:
            ...
            if i + 1 < len(
                substrs
            ):  # Prepend to the next string if there is a next substr
...
duplicates_fcs = {
    seq: indices for seq, indices in seq_indices.items() if len(indices) > 1
}
...
FCS_THINNING = char_lengths.mode()[
    0
]  # the mode of strings length, still feels inappropriate
...
for j in range(0, NUM_CLASSES):  # the hurf
    if top_fcs[str(j)] is not None:
        for i in range(0, len(top_fcs[str(j)])):  # the subsequences
            ...
            if top_fcs[str(j)][i]["score"] / appearance[j] > 0:
                ffcs[j][i] = top_fcs[str(j)][i][
                    "freq"
                ]  # apperance frequency of each substring
                sfcs[j][i] = top_fcs[str(j)][i][
                    "score"
                ]  # length-dependent score of each substring
    ...
    for i, s1_ch in enumerate(s1):
        ...
        for j in range(low, hi + 1):
            ...
            if not s2_flags[j] and s2[j] == s1_ch:
                ...
                break
            if (
                abs(ord(s1[i]) - ord(s2[j])) == 1
                or abs(ord(s1[i]) - ord(s2[j])) == 4
                or abs(ord(s1[i]) - ord(s2[j])) == 7
            ):
...
def draw_heatmap(data, xlabel, ylabel, title):
    ...
    ax = sns.heatmap(
        data, cmap="nipy_spectral", annot=True, cbar=True, annot_kws={"size": 4}
    )
    plt.yticks(ticks=range(NUM_CLASSES), labels=hurf, rotation=0, fontsize=6)
...
def draw_heatmap_slice(data, xlabel, ylabel, title):
    ...
    # Create the heatmap
    ax = sns.heatmap(
        data_to_plot, cmap="nipy_spectral", annot=True, cbar=True, annot_kws={"size": 4}
    )
    ax.set_yticks(range(NUM_CLASSES))
    ax.set_yticklabels(hurf, rotation=0, fontsize=6)
    ax.set_xticks(range(data_to_plot.shape[1]))
    ax.set_xticklabels(
        range(2, data.shape[1]), ha="center", fontsize=6
    )  # Explicitly center-align x-ticks
...
def string2rasm(chaincode):
    ...
    # the MC search
    while len(substrokes) > 1 or (
        len(substrokes) == 1 and len(substrokes[0]) >= LENGTH_MIN
    ):
        ...
        while True:
            ...
            temp = score_mc_acc
            score_mc_acc = np.zeros(
                (NUM_CLASSES, int(len(tee_clean) * PHI + 1)), dtype=float
            )
            score_mc_acc[:, 0 : temp.shape[1]] = temp
            temp = score_mc_mul
            score_mc_mul = np.ones(
                (NUM_CLASSES, int(len(tee_clean) * PHI + 1)), dtype=float
            )
            score_mc_mul[:, 0 : temp.shape[1]] = temp
            temp = score_mc_met
            score_mc_met = np.zeros(
                (NUM_CLASSES, int(len(tee_clean) * PHI + 1)), dtype=float
            )
            ...
            for mc_retry in range(int(MC_RETRY_MAX)):
                ...
                if len(top_fcs[str(mc_class)]) != 0:
                    ...
                    for mc_length in range(
                        mc_length_min, int(len(tee_clean) * PHI + 1)
                    ):
                        # just adjust FACTOR_LENGTH if prefer for longer substrokes
                        score_tee1 = myjaro(tee_clean, fcs_lookup[:mc_length]) * pow(
                            FACTOR_LENGTH, mc_length
                        )
                        score_tee2 = myjaro(
                            reverseFreeman(tee_clean), fcs_lookup[:mc_length]
                        ) * pow(FACTOR_LENGTH, mc_length)
                        score_tee = max(
                            score_tee1, score_tee2
                        )  # some strokes can be written back to front due to branching
                        ...
                        if score_tee > score_mc_met[int(mc_class)][mc_length]:
                            ...
                            score_mc_met[int(mc_class)][mc_length] = (
                                score_tee + score_mc_met[int(mc_class)][mc_length]
                            ) / 2  # incremental metropolis
        ...
        draw_heatmap(
            score_mc_acc,
            "hurf character length",
            "hurf class",
            "cumulative add MC-myjaro "
            + str(int(MC_RETRY_MAX))
            + "/"
            + str(FACTOR_LENGTH)
            + "\n"
            + tee,
        ...
        score_mc_acc_cluster = np.zeros(
            (NUM_CLUSTER, score_mc_acc.shape[1]), dtype=float
        )
        ...
        plt.figure(dpi=300)
        plt.plot(
            score_mc_acc_cluster[cluster_best],
            label=f"cluster[{cluster_best}]",
            color="blue",
        )
        for n in range(len(clusters[cluster_best])):
            plt.plot(
                score_mc_acc[clusters[cluster_best][n]],
                label=f"{hurf[clusters[cluster_best][n]]}",
                color=random_color(),
                linestyle="dashdot",
            )
        plt.legend()
        ...
        if valley_index == -1:
            for i in range(peak_index, len(score) - 1):
                print(
                    f"{i} {(score[i-1]-score[i]):.2f} {score[i]:.2f} {(score[i]-score[i + 1]):.2f}"
                )
                if score_smoothed[i] < score_smoothed[i - 1] and (
                    score[i] < score[i + 1]
                    or (
                        score[i] - score[i + 1] > 0
                        and score[i] - score[i + 1] < score[peak_index] / pow(PHI, 10)
                    )
                ):
                    valley_index = i
                    break
        ...
        tee_clean = re.sub(f"[{re.escape('-+abcABCx')}]", "", tee)
        len_best = min(
            max(valley_index, len_max, peak_index), len(tee_clean), len(score)
        )
        ...
        if hurf_best == "ک" or hurf_best == "ݢ" or hurf_best == "ك" or hurf_best == "ل":
            if "B" in tee_best or "C" in tee_best:
                hurf_best = "ك"
            if (
                "A" in tee_best or "a" in tee_best or "b" in tee_best
            ):  # some styles write the dot either on top or bottom
                hurf_best = "ݢ"
...
def string2rasm_old(chaincode):
    ...
    while len(remainder_stroke) >= 2 and remainder_stroke != "":
        ...
        for mc_retry in range(int(MC_RETRY_MAX)):
            ...
            fcs_lookup = ""
            mc_class = random.randint(
                1, len(top_fcs) - 1
            )  # may also compare to the whole string
            ...
            if len(top_fcs[str(mc_class)]) != 0:
                ...
                for len_mc in range(LENGTH_MIN, len_mc_search):
                    ...
                    if len(top_fcs[str(mc_class)]) != 0:
                        ...
                        score_tee1 = myjaro(
                            remainder_stroke[0:len_mc], fcs_lookup
                        ) * pow(
                            FACTOR_LENGTH, len_mc
                        )  # (optionally)
                        score_tee2 = myjaro(
                            reverseFreeman(remainder_stroke[0:len_mc]), fcs_lookup
                        ) * pow(
                            FACTOR_LENGTH, len_mc
                        )  # (optionally)
                        ...
                        if score_tee > score_mc_met[int(mc_class)][len_mc]:
                            ...
                            score_mc_met[int(mc_class)][len_mc] = (
                                score_tee + score_mc_met[int(mc_class)][len_mc]
                            ) / 2
        ...
        tophurf = [
            [
                max(
                    myjaro(remainder_stroke, lookup),
                    myjaro(reverseFreeman(remainder_stroke), lookup),
                )
                for lookup in string_mc_met[peak]
            ]
            for peak in peaks
        ]
        ...
        # minimum tentative estimate for hurf length
        ...
        if len_best <= LENGTH_MIN * PHI:
            ...
            asymptote = np.mean(
                max_row[-int((len_mc_max - LENGTH_MIN) / PHI) :]
            )  # shall we do row value?
            divergence_val = np.where(
                np.abs(max_row - asymptote) > asymptote / pow(PHI, 4)
            )[0][-1]
            column_variances = np.var(score_mc_met, axis=0)
            column_variances /= np.max(column_variances)  # scale max to 1
            asymptote_var = np.mean(
                column_variances[-int((len_mc_max - LENGTH_MIN) / PHI) :]
            )  # shall we do variance?
            divergence_var = np.where(
                np.abs(column_variances - asymptote_var)
                > asymptote_var / (len_mc_max - LENGTH_MIN) / PHI
            )[0][-1]
        ...
        # terminus hurfs
        if (
            hurf_best == "د" or hurf_best == "ذ" or hurf_best == "ز" or hurf_best == "ۏ"
        ...
        elif (hurf_best == "ا" or hurf_best == "و" or hurf_best == "ر") and len(
            remainder_stroke
        ) - len_best > SUBSTROKE_MIN_LENGTH:
